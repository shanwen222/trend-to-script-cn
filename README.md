...
```mermaid
flowchart LR
    A["输入<br/>行业·平台·篇幅·目标"] --> B["强制依赖闸门<br/>自动安装并验证"]
    B --> C["来源预检"]
    C --> D["实时热榜 / 平台证据 / 网页核验"]
    D --> E["事实锁与热点长尾化"]
...
最少提供三个要素即可开始：行业/垂类、目标平台、交付形式与篇幅（例如“母婴 + 抖音 + 2 分钟口播”）。如果用户说“今天”“日报”或“实时”，时效默认取当天，并在来源覆盖中写明抓取时间。

为了减少返工，建议补充目标受众或年龄段、账号人设、内容目标（涨粉/科普/转化）、语气、是否商业内容以及希望输出几条。缺少这些字段时，Skill 采用最小假设并把假设写出来；不会因为缺少非关键字段而阻塞运行。
